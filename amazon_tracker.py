...
class AmazonApi:
    ...
    def get_price(self):
        ...
        try:
            ...
            try:
                ...
                # if theres no price check availability of item
                if 'Availabe' in availability:
                    price = self.driver.find_elements_by_class_name("olp-padding-right").text
                    price = price[price.find(self.currency):]
                    price = self.convert_price(price)
            except Exception as e:
                print(e)
            print(f"Can't get price of product - {self.driver.current_url}")
            return None
        except Exception as e:
            print(e)
            print(f"Can't get price of product - {self.driver.current_url}")
    ...
    # gets unique product id between dp and ref in the url
    def get_asin(self, product_link):
        return product_link[product_link.find('/dp/') + 4:product_link.find('/ref')]

    def get_products_links(self):
        self.driver.get(self.base_url)
        element = self.driver.find_element_by_xpath('//*[@id="twotabsearchtextbox"]') #find_element_by_id("twotabsearchtextbox")
        ...
        try:
            results = result_list.find_elements_by_xpath(
                "//div/div/div/span/div/div/div[2]/div/div/div/span/a")
                #"//div/div/span/div/div/div/h2/a"
                #"//div/span/div/div/div[2]/div[2]/div/div[1]/div/div/div[1]/h2/a"

            # iterates through all the links in results and stores them
